...
async def create_sku(sku: SkuCreate, session: AsyncSession = Depends(get_session)):
    sku_controller = SkuController(session=session)
...
async def update_sku(
    id: int, sku: SkuUpdate, session: AsyncSession = Depends(get_session)
):
    sku_controller = SkuController(session=session)
...
async def delete_sku(id: int, session: AsyncSession = Depends(get_session)):
    sku_controller = SkuController(session=session)
